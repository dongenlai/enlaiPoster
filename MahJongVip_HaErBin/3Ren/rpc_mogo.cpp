 		...
 		// 各玩家的明牌列表 [{"data": [{}]}] -- 
  		...
  		p->PushPack(V_OBJ_ARY, "mingPai", pvo1);
   		...
   		// 各玩家的 gameInfo [{}]
    			...
    			PushItemToVector(pvo2, V_INT32, "isTrust");
    			PushItemToVector(pvo2, V_INT32, "hasHu");           // 是否已经胡牌
                PushItemToVector(pvo2, V_INT32, "hasTing");         // 是否已经听牌
				PushItemToVector(pvo2, V_INT32, "sPiaoType");		// 用户选择的漂类型（-1：还没有选漂，0：无漂，1：漂1，2：漂2）
    			PushItemToVector(pvo2, V_OBJ_ARY, "huPai", pvo3);   // 胡牌列表 
